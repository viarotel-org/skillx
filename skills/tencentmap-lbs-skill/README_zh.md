...
>   或在对话中直接告知我来配置。
>
> **方式二：申请临时体验 Key（免注册，手机验证即可）**
> 通过手机号验证快速获取临时 Key，14 天有效，覆盖 30 个 WebService API（PV=5000/天，QPS=5/秒）。
>
> **请告诉我您的选择：**
>
> - 回复"我已有 Key"或直接提供 Key → 切换正式模式
> - 回复"申请临时 Key" → 启动临时 Key 申请流程

收到用户明确回复后，再按用户选择继续：

- 用户提供正式 Key → 通过 `tmap-lbs config set-key <key>` 配置或记录 Key，切换正式模式，继续处理请求
- 用户选择申请临时 Key → **必须先读取 `tempkey-guide.md` 获取完整申请流程**（协议展示 → 收集手机号 → send_code → 等待验证码 → create_key → save_config → 持久化到 `~/.tencentmap/tempkey.json`），严格按照其中步骤和错误码表执行，获取 Key 后通过 `tmap-lbs config set-key <key>` 配置，切换正式模式，继续处理请求

#### 临时 Key 说明

通过 tempkey 流程获取的临时 Key 与正式 Key 使用方式完全一致，走 `apis.map.qq.com` 正式通道：

- 有效期 14 天，覆盖 30 个 WebService API（含电动车路线等全部接口，无接口限制）
- PV=5000/天，QPS=5/秒
- Key 持久化在 `~/.tencentmap/tempkey.json`，后续调用自动读取
- 过期后需重新申请

> **场景五（轨迹可视化）**：不需要 API Key，使用 `tmap-lbs trail` 命令即可，无需走 Key 申请流程。
