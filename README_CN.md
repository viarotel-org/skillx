...
同步流程在写入生成输出前会规划所有已启用来源，再将计划中的副本提交到 `skills/`。它只移除之前记录在清单中且当前不再活跃的生成目录，未知目录和受保护的 id 不会被改动。订阅仓库中的符号链接如果解析后仍位于来源仓库内，会被展开为真实文件或目录；不安全、循环、损坏或不支持的符号链接会被跳过并记录在同步摘要中。
